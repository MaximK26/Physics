...
export class AABB {
	constructor(options) {
		options = options || {};
		this.lowerBound = options.lowerBound ? new THREE.Vector3().copy(options.lowerBound) : new THREE.Vector3();
		this.upperBound = options.upperBound ? new THREE.Vector3().copy(options.upperBound) : new THREE.Vector3();
	}
	...
	setFromPoints( points, position, quaternion, skinSize ) {
		...
		// Add offset
		if(position) {
			l.addVectors( position, l );
			u.addVectors( position, u );
		}
		if(skinSize) {
			l.x -= skinSize;
			l.y -= skinSize;
	...
	contains(aabb) {
		var l1 = this.lowerBound, u1 = this.upperBound,
			l2 = aabb.lowerBound, u2 = aabb.upperBound;
		return (
			(l1.x <= l2.x && u1.x >= u2.x) &&
			(l1.y <= l2.y && u1.y >= u2.y) &&
			(l1.z <= l2.z && u1.z >= u2.z)
